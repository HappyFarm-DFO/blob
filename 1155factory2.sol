...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    address public Dev = 0xF0899D85eD919Cf02Ea7243e47c2D4834Bc6dda0;
    address public Curator = 0xF0899D85eD919Cf02Ea7243e47c2D4834Bc6dda0;
    address public Curricular = 0x3a326C47D7471B87360661Cc8Ad070AB87C87756;
    address public CypherShop = 0x4726709CD3470B00A1Ec19F4c2629e07eBdee688;
    ...
    function mintSetPrice(uint256 id,string memory name,string memory symbol,uint256 amount,string memory uri) public returns(bool){
        ...
        index++;
        _mint(Curator,index,100,"");
        _uri[index]=uri;
        ICypherCardShop1155(CypherShop).setPrice(payable(Curator),payable(Curator),address(this),index);
        this.safeTransferFrom(address(this),CypherShop,index,97000000000000000000,"");
        this.safeTransferFrom(address(this),Curator,index,2000000000000000000,"");
        this.safeTransferFrom(address(this),Dev,index,1000000000000000000,"");
    ...
    function mintNow(string memory name,string memory symbol,uint256 amount,string memory uri) public returns(bool){
         require(msg.sender==Curator,"permission requi red");
        index++;
        _mint(Curator,index,100,"");
        _uri[index]=uri;
        ICypherCardShop1155(CypherShop).setPrice(payable(Curator),payable(Curator),address(this),index);
        //IERC20(tokenAddress).transfer(CypherShop, 100000000000000000000);
        this.safeTransferFrom(address(this),CypherShop,index,100000000000000000000,"");
...
contract ICypherCardShop1155{
    ...
    function buy(address token,uint256 id,uint256 amount)public payable returns(bool){
        prices[token][id]+=amount*100000000000000000;
        require(msg.value>=prices[token][id]*amount,"more honey!");
        creators[token][id].transfer(msg.value/3);
        curators[token][id].transfer(msg.value/3);
        CypherVault.transfer(msg.value-(msg.value/3*2));
        //require(IERC20(token).transfer(msg.sender, 1000000000000000000*amount),"wtf?");
        ERC1155(token).safeTransferFrom(address(this),msg.sender,id,1000000000000000000*amount,"");
        //emit scx(token,id);
        return true;
    }
    ...
    function setPrice(address payable curator,address payable author,address vault,uint256 id) external{
        ...
        prices[vault][id]=10000000000000000;
